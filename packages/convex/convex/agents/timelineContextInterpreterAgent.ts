...
import { timelineInterpretationPrompt } from "../ai/prompts";

const agentComponent = (components as { agent: AgentComponent }).agent;
const AGENT_NAME = "Timeline Context Interpreter Agent";
const USE_CASE = "timeline_interpretation";
...
export const timelineContextInterpreterAgent = new Agent<AgentCtx>(
	agentComponent,
	{
		name: AGENT_NAME,
		languageModel,
		instructions: timelineInterpretationPrompt,
		usageHandler: async (ctx, { usage, provider, model, threadId, agentName }) => {
			...
			await ctx.runMutation(internal.ai.telemetry.recordUsage, {
				...
				response: undefined,
				metadata: { source: "timeline-interpretation", trackedBy: "usageHandler" },
			});
		},
	}
);
