...
export function ContextSnapshotCard({
	...
	const latestContextRun = useQuery(
		...
		snapshot ? { productId, useCase: "context_agent" } : "skip",
	);
	const latestFeatureRun = useQuery(
		api.agents.agentRuns.getLatestRunForUseCase,
		snapshot ? { productId, useCase: "feature_scout" } : "skip",
	);
	const latestCapabilityRun = useQuery(
		api.agents.agentRuns.getLatestRunForUseCase,
		snapshot ? { productId, useCase: "capability_aggregator" } : "skip",
	);
		...
		<Card>
			<CardHeader className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
				...
				<div className="flex flex-wrap items-center gap-2">
					...
					>
						{isContextRunning
							? t("context.contextAgentGenerateRunning")
					...
					>
						{isFeatureRunning
							? t("context.featureScoutGenerateRunning")
					...
					<Button
						...
						disabled={isCapabilityRunning}
					>
						{isCapabilityRunning
							? t("context.capabilityAggregatorGenerateRunning")
							: t("context.capabilityAggregatorGenerate")}
					</Button>
				</div>
			</CardHeader>
